...
func (cr *CartRepository) AddToCart(userID string, product models.Product) error {
	...
	row := cr.db.QueryRow("SELECT ID FROM cart_items WHERE cart_id=? AND product_id=?", cartID, product.ID)
	var id string
	...
	_, err = cr.db.Exec("UPDATE cart_items SET quantity = quantity + 1 WHERE id = ?", id)
	return err
}
...
func (cr *CartRepository) GetCartIDByUserID(userID string) (string, error) {
	row := cr.db.QueryRow("SELECT cart_id FROM cart WHERE user_id = ?", userID)
	var cartID string
	err := row.Scan(&cartID)
	if err != nil {
		return "", err
